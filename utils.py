...
tf.app.flags.DEFINE_string('train_lst', './train.txt', 'the train data list')
tf.app.flags.DEFINE_string('val_lst', './val.txt', 'the train data list')
...
class DataIterator:
    def __init__(self, data_lst):
        ...
        with open(data_lst, 'r') as s:
            data_content = s.readlines()

        for data in data_content:
            [filename, label] = data.split(":")
            ...
            label = list(map(lambda s: int(s), label.split(',')))
